...
export async function buildRouteWithState(options = {}) {
  ...
  try {
    ...
    // Гарантированный хук после успешной загрузки маршрута (requestsuccess)
    // ------------------------------
    try {
      const model = multiRoute.model;
      if (model && model.events && typeof model.events.add === 'function') {
        model.events.add('requestsuccess', () => {
          // На этом событии геометрия и boundedBy уже доступны стабильнее всего
    ...
    // Обновление маршрута → рамки + альтернативы (оставляем)
    ...
    // Страховка: даже если события сработают до готовности,
    // через небольшой таймаут повторим пересчет рамок.
    if (hasWindow) {
      setTimeout(() => {
        try {
...
function normalizeCoordPair(pair) {
  ...
  // Если первое значение явно longitude (>|90|), значит уже [lon, lat]
  if (Math.abs(a) > 90 && Math.abs(b) <= 90) return [a, b];

  // Если второе значение явно longitude (>|90|), значит вход [lat, lon]
  if (Math.abs(b) > 90 && Math.abs(a) <= 90) return [b, a];

  // Для маршрутов ymaps чаще всего [lat, lon] → [lon, lat]
  return [b, a];
}
...
export function refreshFramesForActiveRoute(fallbackPoints) {
  ...
  // Совместимость: если тумблера нет (или он скрыт) — считаем, что включено
  ...
  if (framesSvc && !isCarMode()) {
    try {
      ...
      const normalizedRoutePoints = Array.isArray(routePoints)
        ? routePoints.map(normalizeCoordPair).filter(Boolean)
        : [];

      const truckParams = window.__TT_TRUCK_PARAMS || {};
      const res = framesSvc.updateFramesForRoute(normalizedRoutePoints, truckParams);
